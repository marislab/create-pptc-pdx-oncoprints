...
colnames(onco.matrix) %in% colnames(cn.matrix)
dim(onco.matrix)
dim(cn.matrix)
#### Rearranges CN file into same order as the onco file ####
#### Removes Genes that aren't present in Onco file ####
cn.matrix.rearranged=cn.matrix[match(rownames(onco.matrix),rownames(cn.matrix)),]

#### Determine Genes in cn but not in onco ####
Unmatched_Genes = setdiff(rownames(cn.matrix),rownames(onco.matrix))

#### Create matrix with unmatched brain cn genes to be appended to rearranged matrix ####
Unmatched_cn_Gene_matrix = cn.matrix[rownames(cn.matrix) %in% Unmatched_Genes,]
