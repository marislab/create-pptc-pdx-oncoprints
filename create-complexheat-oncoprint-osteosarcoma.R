...
samples <- subset(clin.class, Histology.Oncoprints == broad.hist & Tumor_Sample_Barcode != "<NA>")
...
#rename to Histology for legend printing
 ...
 df_tp53 <- sample_df[colnames(mat.clean), "tp53_score", drop = FALSE]
 df_age <- sample_df[colnames(mat.clean), "Age", drop = FALSE]
 
 #combine all
 df_anno <- cbind(df_hist, df_phase, df_sex, df_age, df_tp53)
...
heat_anno = HeatmapAnnotation(df = df_anno,
                                col = list(Histology = histcol,
                                           ...
                                           #Age = colorRamp2(c(0, quantile(df_anno$Age, na.rm = T, probs=1/4), 
                                           #                  median(df_anno$Age, na.rm = T), 
                                           #                 quantile(df_anno$Age, na.rm = T, probs=3/4),
                                           #                max(df_anno$Age, na.rm = T)), 
                                           tp53_score = colorRamp2(c(0, 1), c("#FFFF00", "#0000FF"))), 
                                annotation_height = 1,
                                na_col = "whitesmoke")
